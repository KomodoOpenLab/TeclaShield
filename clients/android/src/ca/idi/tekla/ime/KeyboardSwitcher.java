...
public class KeyboardSwitcher {
    ...
    KeyboardSwitcher(TeklaIME context) {
        mContext = context;
    ...
    void setInputView(TeklaKeyboardView inputView) {
    ...
    boolean onKey(int key) {
        ...
        switch (mSymbolsModeState) {
            case SYMBOLS_MODE_STATE_BEGIN:
                if (key != TeklaIME.KEYCODE_SPACE && key != TeklaIME.KEYCODE_ENTER && key > 0) {
                    mSymbolsModeState = SYMBOLS_MODE_STATE_SYMBOL;
                }
                break;
            case SYMBOLS_MODE_STATE_SYMBOL:
                if (key == TeklaIME.KEYCODE_ENTER || key == TeklaIME.KEYCODE_SPACE) return true;
                break;
        }
        return false;
    }
